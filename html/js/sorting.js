		...
		jQuery('.btn_load_more').click(function(){
			...
			if (now_load > 0) {
				if (addon_options.type == 0) {
					...
					for (i = i_start-1; i < i_start+now_load-1; i++) {
						loaded_object = loaded_object + '<div class="element '+ addon_options.items[i].category +'"><div class="portfolio_item"><div class="hover_img"><img src="'+ addon_options.items[i].src +'" alt="" /><a class="zoom" href="'+ addon_options.items[i].src +'" rel="prettyPhoto[portfolio1]"></a></div><div class="item_descr">'+ addon_options.items[i].title +'</div></div></div>';
					}
				}
